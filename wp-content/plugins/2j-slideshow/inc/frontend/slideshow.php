	...
	*/
	public function addJavaScriptStyle($styleValue, $styleName = '', $hover='1'){
 	...
 	function getImage($img){
 		...
 		$cleatTitle = htmlspecialchars($img['data']->post_title);
		...
		if( $img['link'] && ( $img['typelink']==1 || $img['typelink']==2) ){
			...
			'<div  '
				...
				.'>'
					...
					'<div '
							.'class="twoj_desc_base'
							.' twoj_desc_'.$img['position'].$img['size']
							.' twoj_desc_margin'.$img['padding']       
						.'">'
						.'<div class="twoj_desc_header twoj_desc_'.$img['style'].$img['opacity'].'">'.$img['data']->post_title.'</div>'
						.'<div class="twoj_desc_text twoj_desc_'.$img['style'].$img['opacity'].'">'.$img['data']->post_content.'</div>'
					.'</div>' : '')
			.'</div>';
		} else {
			$imageEl .= 	
